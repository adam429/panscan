__TASK_NAME__ = "uniswap/simulation_v8"
...
def main()
    ...
    pool_option = [""]+DataStore.get("all_pools").map {|p| "#{p[:token0]}/#{p[:token1]} - #{p[:dex_fee]*100}% pool"}
    pool_option_value = [""]+DataStore.get("all_pools").map {|p| p[:pool]}
    ...
    RenderWrap.html= <<~EOS
        ...
        <%= text binding: :status %> | [<%= button text:"update chart", action:"update_chart()" %>]
        <br/><br/>
      ...
      <div id="container">
        ...
        <div>
          ...
          <br/><%= slider min:-100, max:100, value:-20, binding: :price_a_mul %>  <br/>
          Upper Price: <%= text binding: :price_b %>
          <br/><%= slider min:-100, max:100, value:20, binding: :price_b_mul %> <br/>
          ...
          [<%= button text:"clean liquidity", action:"$data['sim'].uni.clean_liquidity('user');" %>]</br></br>
...
$document.ready do    
    ...
    if $data["sim"].sim_data!=nil and  $data["sim"].sim_data[0]!=nil then
        data_column = $data["sim"].sim_data[0].map {|k,v| k}
