__TASK_NAME__ = "uniswap/simulation_runner_v1"
...
#        $logger.call "===binding_slider==="
          ...
          if $saved_sim_time != $vars['sim_time'] then
            ...
            #   $logger.call "clean_timeout"
            #   $logger.call $sim_time_timeout

              calculated_var_update_all()
              
              $sim_time_timeout = $$[:setTimeout].call(->{
                # $logger.call "change_time #\{new_time\}"
                $data["sim"].change_time(new_time)
                calculated_var_update_all()
                # update_chart()
              },500)
            #   $logger.call "set_timeout"
            #   $logger.call $sim_time_timeout
