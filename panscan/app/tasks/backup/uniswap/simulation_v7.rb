__TASK_NAME__ = "uniswap/simulation_v7"
...
def main()
    ...
    RenderWrap.html= <<~EOS
        ...
        [<%= button text:"reload", action:"reload()" %>] [<%= button text:"save", action:%( update_task({update_params:{sim_data:$data["sim"].to_encode_str()}}) ) %>]
        <%= text binding: :status %>
        <br/><br/>
      ...
      <div id="container">
        ...
        <div>
          ...
          <%= slider min:0, max:data[:sim].dex.count-1, value:data[:sim].sim_time, binding: :sim_time %> 
          ...
          <%= slider min:0, max:data[:sim].dex.count-1, value:data[:sim].sim_time_end, binding: :sim_time_end %> 
