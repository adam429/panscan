__TASK_NAME__ = "uniswap/simulation_v3_look"
...
document.getElementById("dwn-btn").addEventListener("click", function(){
    ...
    def generate_data()
        ...
        ($vars[:sim_time].to_i..time_end).each do |time| 
            $logger.call "time - #\{time\}"
            $data["sim"].change_time(time,true)
        ...
        $vars['export_data'] = format_table(data, [:id, :time, :price, :token0_amt, :token1_amt, :token0_fee, :token1_fee])
        $vars['export_data_csv'] = format_csv_table(data, [:id, :time, :price, :token0_amt, :token1_amt, :token0_fee, :token1_fee])
    end
