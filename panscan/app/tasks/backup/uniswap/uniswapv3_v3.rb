__TASK_NAME__ = "uniswap/uniswapv3_v3"

load(Task.load("base/render_wrap"))

class UniswapV3 < MappingObject
    def self.task
        return "uniswap/uniswapv3_v3"
    end
end
...
class Pool< MappingObject
    ...
    def load_from_redis(pool_id,uni,reversed=false)
        ...
        self.init_tick = DataStore.get("uniswap.#{pool_id}.init_tick")
        ...
        if reversed then
            ...
            self.init_tick = -1*self.init_tick
        end
...
$profiler[:slice_liquidity_pool] = ($profiler[:slice_liquidity_pool] or 0) + (Time.now()-profiler_time2)
        ...
        # $logger.call " slice_pool.size = #{slice_pool.size}"
        # $logger.call " ul = #{ul}"
        # $logger.call " l = #{l}"
