__TASK_NAME__ = "uniswap/hedge"

load(Task.load("base/render_wrap"))

class Hedge < MappingObject
    def self.task
        return "uniswap/hedge"
    end
...
def main
    hedge = Hedge.new
    hedge.init("ETH","USDT",0.0004)

    hedge.set_price(3000)
    hedge.adj_position(-10,"group_a")
    hedge.stats
    hedge.set_price(2000)
    hedge.stats
    hedge.adj_position(-10,"group_b")
    hedge.stats
    hedge.set_price(1000)
    hedge.stats
    hedge.adj_position(10,"group_c")
    hedge.stats
    hedge.set_price(0)
    hedge.stats
    hedge.set_price(4000)
    hedge.stats    
    
    RenderWrap[:hedge]=hedge
    RenderWrap.data
end
