__TASK_NAME__ = "panbot/simulation/panbot_interval_simulation_matrix_result"

Task.load("base/database",binding)

def main()
    task = Task.where(name:"panbot/simulation/panbot_simulation_matrix").order(updated_at: :desc).where("tid is not null").first
    result = task.raw_ret.map {|x| Task.find(x) }
    result = result.map {|x|
       json = JSON.parse(x.params)
