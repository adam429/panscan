...
def stats(bet_result,epoch_begin,epoch_end)
    ...
    bet_result = bet_result.map { |x| 
        x[:return_amt] = -1*x[:amount] + ( x[:win_bet]==1 ? (0.97 * x[:amount] * ( x["#{x[:bet_result]}_payout".to_sym] or 0 ) ) : 0 ) if x[:bet]!=""
        x
    }

    
    invest_cnt = bet_cnt = bet_epoch = bet_result.filter {|x| x[:bet]!=""}.count
    all_epoch = epoch_end-epoch_begin+1
    bet_ratio = bet_epoch/all_epoch.to_f
    bet_round_payout = bet_result.filter {|x| x[:bet]!=""}.map { |x| [x[:bull_payout],x[:bear_payout]].max }.sum / bet_epoch.to_f
    bet_bull_cnt = bet_result.filter {|x| x[:bet]=="bull"}.count
    bet_bull_ratio = bet_bull_cnt/bet_epoch.to_f
    
    avg_bet_amt = bet_result.filter {|x| x[:bet]!=""}.map {|x| x[:amount]}.sum / bet_epoch.to_f
    avg_last_block_order = bet_result.filter {|x| x[:bet]!=""}.map {|x| x[:bet_block_order]}.sum / bet_epoch.to_f
    
    right_bet_ratio = bet_result.filter {|x| x[:bet]!=""}.map {|x| x[:right_bet]}.sum / bet_epoch.to_f
    win_bet_ratio = bet_result.filter {|x| x[:bet]!=""}.map {|x| x[:win_bet]}.sum / bet_epoch.to_f
    
    invest_amt = bet_result.filter {|x| x[:bet]!=""}.map {|x| x[:amount]}.sum
    return_amt = bet_result.filter {|x| x[:bet]!=""}.map {|x| x[:return_amt]}.sum
    return_flow = return_amt / all_epoch.to_f
    
    max_retrace = 0
    running_return = 0
    bet_result.filter {|x| x[:bet]!=""}.map {|x| x[:return_amt]}.each do |x|
