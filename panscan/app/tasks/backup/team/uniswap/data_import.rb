__TASK_NAME__ = "team/uniswap/data_import"
...
(begin_param..end_param).each do |cp|
    ...
    DataStore.set("uniswap.#{cur_pool[:pool]}",cur_pool)
    DataStore.set("uniswap.#{cur_pool[:pool]}.pool",pool.pool)
    DataStore.set("uniswap.#{cur_pool[:pool]}.swap",swap)
    DataStore.set("uniswap.#{cur_pool[:pool]}.init_tick",pool.init_tick)
    DataStore.set("uniswap.#{cur_pool[:pool]}.time_table",timetable)

    $logger.call " uniswap.#{cur_pool[:pool]} - #{JSON.dump(cur_pool).size} "
    $logger.call " uniswap.#{cur_pool[:pool]}.pool - #{JSON.dump(pool.pool).size} "
    $logger.call " uniswap.#{cur_pool[:pool]}.swap - #{JSON.dump(swap).size} "
    $logger.call " uniswap.#{cur_pool[:pool]}.init_tick - #{JSON.dump(pool.init_tick).size} "
    $logger.call " uniswap.#{cur_pool[:pool]}.time_table - #{JSON.dump(timetable).size} "
    nil
end
