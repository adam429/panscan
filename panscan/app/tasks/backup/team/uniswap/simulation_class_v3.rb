__TASK_NAME__ = "team/uniswap/simulation_class_v3"
...
class Simulation < MappingObject
    def self.task
        return "team/uniswap/simulation_class_v3"
    end
  ...
  "vconcat": [
    ...
    def change_time(new_time,run=false)
        ...
        if self.pool.swap.size>0 then
            ...
            self.user_pool = self.uni.liquidity_pool.filter {|x| x[:sender]=="user"}
            self.uni.liquidity_pool = self.pool.calc_pool(block_number,self.user_pool)
        end
    ...
    def simulate_tick_logic(time,time_end)
        ...
        bot_data = self.bot.run(self.cex, time, time_ts, time_str,self.uni.price,token0_amt,token1_amt)
        ...
        total_pnl = total_fee + value_diff - cex_fee
        ...
        column = [:time,:price,:token0_amt,:token1_amt,:token0_fee,:token1_fee,:total_fee, :dex_value, :ddex_value, :cex_position, :cex_value,:value_diff,:dprice_percent,:value_diff_dex_value_percent,:cex_fee,:total_pnl,:roi_percent,:unhedged_pnl,:unhedged_roi_percent, :bot_output,:volume0,:volume1,:volume,:ul_ratio]
